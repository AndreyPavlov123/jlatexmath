...
class GeomFactoryAndroid : GeomFactory() {
    ...
    override fun createLine2D(
        x1: Double, y1: Double, x2: Double, y2: Double
    ) = Line2DA(x1, y1, x2, y2)

    override fun createRectangle2D(
        x: Double, y: Double, width: Double, height: Double
    ) = Rectangle2DA(x, y, width, height)

    override fun createRoundRectangle2D(
        x: Double, y: Double, w: Double,
        h: Double, arcw: Double, arch: Double
    ) = RoundRectangle2DA(x, y, w, h, arcw, arch)

    override fun createPoint2D(x: Double, y: Double) = Point2DA(x, y)

    override fun createArea(s: Shape?) = when (s) {
        is AreaA -> s.duplicate()
        is ShapeA -> AreaA(Path(s.path))
        else -> AreaA()
    }

    override fun newArea() = AreaA()
}